...
export async function POST(request) {
  ...
  let attachments = files.map((file, index) => {
    return {
      ...
      filename: index === 0 ? data?.resumeFileName : data?.licenceFileName,
  ...
  const res = await sendgrid.send({
    ...
    <html lang="en">
      <head>
        ...
        <meta name="description" content="Delore Care Job Application" />
              ...
              <td align="center">
                ...
                >
                  <tbody style="margin-left: 20px; margin-right: 20px">
                    <h3>
                      You've got a new mail from ${data.fName} ${data.oName} ${
      data.lName
